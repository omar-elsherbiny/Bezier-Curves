...
def main(drawing=False):
    ...
    DRAW=drawing
    T=0
    DRAG1,DRAG2,DRAG3,DRAG4,DRAG5,DRAG6=False,False,False,False,False,False
    ...
    CC=get_complete(P1,P2,P3)
    #
    P4=pyg.rect.Rect(int(SCREEN_WIDTH/2), int(SCREEN_HEIGHT/3*2), SIZE, SIZE)
    P5=pyg.rect.Rect(int(SCREEN_WIDTH/4*3), int(SCREEN_HEIGHT/3*2), SIZE, SIZE)
    P6=pyg.rect.Rect(int(SCREEN_WIDTH/4*3), int(SCREEN_HEIGHT/4*3.5), SIZE, SIZE)
    CC2=get_complete(P2,P3,P4)
    CC3=get_complete(P3,P4,P5)
    CC4=get_complete(P4,P5,P6)
    ...
    while run:
        for event in pyg.event.get():
            if event.type == pyg.QUIT:
                ...
                if event.button == 1:            
                    ...
                    elif P5.collidepoint(event.pos):
                        DRAG5 = True
                        mouse_x, mouse_y = event.pos
                        offset_x = P5.x - mouse_x
                        offset_y = P5.y - mouse_y
                    elif P6.collidepoint(event.pos):
                        DRAG6 = True
                        mouse_x, mouse_y = event.pos
                        offset_x = P6.x - mouse_x
                        offset_y = P6.y - mouse_y
                ...
                if DRAG1 or DRAG2 or DRAG3 or DRAG4 or DRAG5 or DRAG6:
                    CC.clear()
                    CC=get_complete(P1,P2,P3)
                    mouse_x, mouse_y = event.pos
                    #
                ...
                if DRAG1:
                    ...
                    P5.x = mouse_x + offset_x
                    P5.y = mouse_y + offset_y
                elif DRAG6:
                    P6.x = mouse_x + offset_x
                    P6.y = mouse_y + offset_y
        ...
        for cord in CC2:
            pyg.draw.circle(SCREEN, (255,255,255), cord, int(SIZE/15))
        for cord in range(max(len(CC),len(CC2))):
            pyg.draw.circle(SCREEN, (0,255,255), (int((CC[cord][0]+CC2[cord][0])/2),int((CC[cord][1]+CC2[cord][1])/2)), int(SIZE/15))
        for cord in range(max(len(CC),len(CC2))):
            pyg.draw.circle(SCREEN, (0,255,255), (int((CC2[cord][0]+CC3[cord][0])/2),int((CC2[cord][1]+CC3[cord][1])/2)), int(SIZE/15))
        pyg.draw.rect(SCREEN, (240, 240, 30), P4)
        pyg.draw.rect(SCREEN, (30, 30, 240), P5)
        pyg.draw.rect(SCREEN, (240, 30, 240), P6)
